var panel_CommunityHouse = {
    ...
    //新房
    findAllCommunity: function (curr) {
        $.ajax({
            type: 'POST',
    ...
    findAllCommunity2: function (curr) {
        $.ajax({
            type: 'POST',
            url: urlScope + '/CommunityDetail/findToHouTai',
            dataType: 'json',
            ...
            success: function (data) {
                ...
                // var str_btn = '<a href="javascript:;" onclick=panel_CommunityHouse.communityhouseAdd("添加新房","communityhouse-list-add.html","800","500") class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加新房</a>';
                // var str_btn =
                //     '<a href="javascript:;" onclick="panel_CommunityHouse.findAllCommunity()"  class="btn btn-primary a2 radius"> 内部小区</a>&nbsp;&nbsp;' +
                //     '<a href="javascript:;" onclick="panel_CommunityHouse.findAllCommunity2()"  class="btn  a2 radius"> 外部小区</a>';
                // '<a href="javascript:;" onclick=panel_CommunityHouse.communityhouseAdd("添加新房","communityhouse-list-add.html","800","500") class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加新房</a>';
                var str_table = '<thead>' +
                    '<tr class="text-c">' +
                    '<th width="30">序号</th>' +
                ...
                for (var i = 0; i < result_data.length; i++) {
                    ...
                    str_table += '</td>' +
                        '<td class="td-manage">';
                    if (result_data[i].detailStatus == '1') {
                    ...
                    if (result_data[i].detailStatus == '2' || result_data[i].detailStatus == '0' || result_data[i].detailStatus == '4') {
                        str_table += '<a href="javascript:;" onclick=panel_CommunityHouse.community_see2(' + result_data[i].id + ',' + result_data[i].detailStatus + ') class="label-success radius label">优惠活动</a>';
                    }
                    // if (result_data[i].detailStatus == '4') {
                    //     str_table += '<a href="javascript:;" onclick=panel_CommunityHouse.community_see(' + result_data[i].id + ',' + result_data[i].detailStatus + ') class="label-success radius label">优惠活动</a>';
                    // }
                    str_table += '</td>' +
                        // '<td class="td-manage">' +
                        // '<a href="javascript:;" onclick=panel_CommunityHouse.revis("修改","communityhouse-list-revise.html?revisid=' + result_data[i].id + '&communityId=' + result_data[i].communityId + '","1000","700") class="label-success  radius label">修改</a>' +
                        // '</td>' +
                        '</tr>';
                }
    ...
    communitySecondAdd: function (title, url, w, h) {
        layer_show(title, url, w, h);
    },

    revis: function (title, url, w, h) {
        layer_show(title, url, w, h);
    },
        ...
        switch (detailStatus) {
            ...
            case 2 :
                ...
                layer.confirm('请仔细确认是否为通过还是驳回？', {
                    ...
                    // if ($("#recommendPercentage") == "") {
                    //     layer.msg("请填写未通过原因", {icon: 1, time: 1000});
                    //     return;
                    // }
                    // $.ajax({
                    //     type: 'POST',
    ...
    community_see: function (id, detailStatus) {
        switch (detailStatus) {
            case 0 :
                layer.confirm('请先在操作栏将状态修改为启用后，才可添加优惠活动', {
                ...
                break;
            case 2 :
                layer.confirm('请先在操作栏将待审核通过，然后在操作栏将状态修改为启用后，才可添加优惠活动', {
                    shadeClose: true, //开启遮罩关闭
                    closeBtn: 0, //不显示关闭按钮
                    title: '优惠活动提示'
                });
            case 4 :
                layer.confirm('审核未通过，暂时还不添加优惠活动', {
                    shadeClose: true, //开启遮罩关闭
                    closeBtn: 0, //不显示关闭按钮
                    title: '优惠活动提示'
                });
    ...
    community_see2: function (id, detailStatus) {
        switch (detailStatus) {
            case 0 :
    ...
    packQueryData_Community: function (data, curr) {
        ...
        var result_data = result_object.data;
        // var str_btn = '<a href="javascript:;" onclick=panel_CommunityHouse.communityhouseAdd("添加新房","communityhouse-list-add.html","800","500") class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加新房</a>';
        var str_btn = '<a href="javascript:;" onclick="panel_CommunityHouse.findAllCommunity()"  class="btn btn-primary a2 radius"> 内部户型</a>&nbsp;&nbsp;' +
            '<a href="javascript:;" onclick="panel_CommunityHouse.findAllCommunity2()"  class="btn a2  radius"> 外部户型</a>'+
        '<a href="javascript:;" id="xiangmu" onclick=panel_CommunityHouse.communityhouseAdd("添加户型","communityhouse-list-add.html","800","500") class="btn  ">添加户型</a>';;
        var str_table = '<thead>' +
            '<tr class="text-c">' +
            '<th width="30">序号</th>' +
            '<th width="100">小区封面</th>' +
        ...
        for (var i = 0; i < result_data.length; i++) {
            ...
            str_table += '</td>' +
                '<td class="td-manage">';
            if (result_data[i].detailStatus == '1') {
                str_table += '<a href="javascript:;" onclick=panel_CommunityHouse.goodActivety_edit("优惠活动","good-activety.html?communityId=' + result_data[i].communityId + '&id=' + result_data[i].id + '","1","1000","700") class="label-success  radius label">优惠活动</a>';
            }
            if (result_data[i].detailStatus == '2' || result_data[i].detailStatus == '0' || result_data[i].detailStatus == '4') {
                str_table += '<a href="javascript:;" onclick=panel_CommunityHouse.community_see(' + result_data[i].id + ',' + result_data[i].detailStatus + ') class="label-success radius label">优惠活动</a>';
            }
